...
/*   Updated: 2024/03/22 15:34:41 by eboulhou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void parceConfFile::read_conf_file() {
	...
	if (!file) {
		throw(std::runtime_error("Config file can not open"));
	}
	std::string input;
...
void parceConfFile::add_the_necessary_data() {
	...
	for (size_t i = 0; i < server.size(); i++) {
		...
		if (server[i].root.empty())
			server[i].root = "/Users/eboulhou/Desktop/website";
		...
		if (check_default_location == false) {
			Location loc;
			loc.path = "/Users/eboulhou/Desktop/website";
			loc.name = "/";
